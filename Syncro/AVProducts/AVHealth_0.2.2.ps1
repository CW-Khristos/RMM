...
.CHANGELOG
    ...
    0.1.9 Optimization and more bugfixes
          ...
          Added '$script:ncxml<vendor>' variables for assigning static 'fallback' sources for AV Product XMLs; XMLs should be uploaded to NC Script Repository and URLs updated (Begin Ln166)
    ...
    0.2.2 Optimization and more bugfixes; code cleanup and enhanced diagnostic output
          ...
          Added 'Pop-Warnings' function to populate '$script:avwarn' hashtable for tracking AV details which did not pass checks
...
#REGION ----- FUNCTIONS ----
  ...
  function Get-AVXML {                                                                              #RETRIEVE AV VENDOR XML FROM GITHUB
    ...
    $script:diag += "Loading : '$($src)' AV Product XML`r`n"
...
if (-not ($script:blnAVXML)) {
  ...
  Rmm-Alert -Category "AV Health : $($i_PAV) : Warning" -Body "$($script:diag)"
      ...
      if (($avs[$av].display -ne $null) -and ($avs[$av].display -ne "")) {
        ...
        if (($avs[$av].display -match $i_PAV) -or 
          ...
          if ($rtWARN) {
            $rtWARN = $false
            $script:blnWARN = $true
            Pop-Warnings $script:avwarn $($avs[$av].display) "Real-Time Scanning :`r`m$($script:o_RTstate)`r`n"
          }
          ...
          if ($tamperWARN) {
            $tamperWARN = $false
            $script:blnWARN = $true
            Pop-Warnings $script:avwarn $($avs[$av].display) "Tamper Protection :`r`n$($tamper)`r`n"
          }
          ...
          if ($scanWARN) {
            $scanWARN = $false
            $script:blnWARN = $true
            Pop-Warnings $script:avwarn $($avs[$av].display) "Last Scan :`r`n$($scans)`r`n"
          }
          ...
          if ($defWARN) {
            $defWARN = $false
            $script:blnWARN = $true
            Pop-Warnings $script:avwarn $($avs[$av].display) "Definition Status :`r`n$($script:o_DefStatus)`r`n"
          }
...
$script:diag += "`r`nDevice Info :`r`nDevice : $($script:computername)`r`n"
$script:diag += "Operating System : $($script:OSCaption) ($($script:OSVersion))`r`n"
#AV DETAILS
...
$script:diag += "`r`nComponent Versions :`r`n$($o_compver)`r`n"
#REAL-TIME SCANNING & DEFINITIONS
write-output "Definitions :" -foregroundcolor yellow
write-output "Status : $($script:o_DefStatus)" -foregroundcolor $ccode
$script:diag += "`r`nDefinitions :`r`nStatus : $($script:o_DefStatus)`r`n"
#THREATS
...
$script:diag += "`r`nCompetitor AV :`r`nAV Conflict : $($script:o_AVcon)`r`n$($script:o_CompAV)"
$script:diag += "`r`nCompetitor Path :`r`n$($script:o_CompPath)`r`nCompetitor State :`r`n$($script:o_CompState)"
#Stop script execution time calculation
StopClock
#SYNCRO OUTPUT
if ($script:blnWARN) {
  ...
  Log-Activity -Message "AV Health : $($i_PAV) : Healthy" -EventName "$($script:diag)"
}
#END SCRIPT
#------------