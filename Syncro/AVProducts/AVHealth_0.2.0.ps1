...
$allout += "`r`nDevice Info :`r`nDevice : $($computername)`r`n"
$allout += "Operating System : $($OSCaption) ($($OSVersion))`r`n"
#AV DETAILS
...
$allout += "`r`nAV Details :`r`nAV Display Name : $($o_AVname)`r`nAV Path : $($o_AVpath)`r`n"
...
$allout += "`r`nDefinitions :`r`nStatus : $($o_DefStatus)`r`n"
#THREATS
...
$allout += "`r`nActive Detections :`r`n$($o_Infect)`r`nDetected Threats :`r`n$($o_Threats)`r`n"
#COMPETITOR AV
...
$allout += "`r`nCompetitor AV :`r`nAV Conflict : $($o_AVcon)`r`n$($o_CompAV)"
$allout += "`r`nCompetitor Path :`r`n$($o_CompPath)`r`nCompetitor State :`r`n$($o_CompState)"
