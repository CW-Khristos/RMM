...
.CHANGELOG
    ...
    0.1.9 Optimization and more bugfixes
          ...
          Added '$script:ncxml<vendor>' variables for assigning static 'fallback' sources for AV Product XMLs; XMLs should be uploaded to NC Script Repository and URLs updated (Begin Ln159)
        ...
        try {
          ...
          try {
            ...
            try {
              ...
              $xmldiag += "BITS.Transfer() - Could not download $($srcAVP)`r`n"
              write-output "BITS.Transfer() - Could not download $($srcAVP)"
              $script:defstatus = "Unknown (WMI Check)`r`nUnable to download AV Product State XML"
              $script:rtstatus = "Unknown (WMI Check)`r`nUnable to download AV Product State XML"
              $script:blnPSXML = $false
            }
          }
        }
