...
.CHANGELOG
    ...
    0.1.9 Optimization and more bugfixes
          ...
          Added '$ncxml<vendor>' variables for assigning static 'fallback' sources for AV Product XMLs; XMLs should be uploaded to NC Script Repository and URLs updated (Begin Ln164)
...
#REGION ----- FUNCTIONS ----
          ...
          if ($itm.name -notmatch "#comment") {                                                     #AVOID 'BUG' WITH A KEY AS '#comment'
            if ($i_PAV -notmatch "Trend Micro") {                                                   #BUILD HASHTABLE FOR ALL AV PRODUCTS EXCEPT TREND MICRO
              ...
              $hash = @{
                ...
                path = "$($itm.$bitarch.path)"
                pathval = "$($itm.$bitarch.pathval)"
                corever = "$($itm.$bitarch.corever)"
                coreverval = "$($itm.$bitarch.coreverval)"
                vcver = "$($itm.$bitarch.vcver)"
                vcverval = "$($itm.$bitarch.vcverval)"
                compver = "$($itm.$bitarch.compver)"
                compverval = "$($itm.$bitarch.compverval)"
                stat = "$($itm.$bitarch.stat)"
                statval = "$($itm.$bitarch.statval)"
                update = "$($itm.$bitarch.update)"
                updateval = "$($itm.$bitarch.updateval)"
  ...
  function SEP-Map {                                                                                #INTERPRET SYMANTEC ENDPOINT PROTECTION DETECTION CODES
    param (
      $intval
    )
    switch ($intval) {
      ...
      if (($avs[$av].display -ne $null) -and ($avs[$av].display -ne "")) {
        ...
        if (($avs[$av].display -match $i_PAV) -or 
          ...
          if ($i_PAV -notmatch "Trend Micro") {
            ...
            $i_verkey = $pavkey[$node].corever
            $i_verval = $pavkey[$node].coreverval
            $i_vckey = $pavkey[$node].vcver
            $i_vcval = $pavkey[$node].vcverval
          }
          #AV PRODUCT COMPONENTS VERSIONS
          $i_compverkey = $pavkey[$node].compver
          if ($i_PAV -match "Trend Micro") {
            $i_compverval = $pavkey[$node].compverval
          }
          #AV PRODUCT STATE
          ...
          if ($i_PAV -notmatch "Trend Micro") {                                                     #HANDLE ALL AV PRODUCTS EXCEPT TREND MICRO
            ...
            try {
              ...
              write-host "$($avs[$av].display) reports '$($o_AVVersion)' for 'Up-To-Date' (Expected : '$($compverkey[0])')" -foregroundcolor yellow
              write-host "$($avs[$av].display) reports '$($vckey.$i_vcval)' for 'Up-To-Date' (Expected : '$($compverkey[1])')" -foregroundcolor yellow
              if (([version]$($o_AVVersion) -ge [version]$($compverkey[0])) -and 
                ([version]$($vckey.$i_vcval) -ge [version]$($compverkey[1]))) {
                  $o_AVStatus = "Up-to-Date : $($true) (REG Check)`r`n"
              } elseif (([version]$($o_AVVersion) -lt [version]$($compverkey[0])) -or 
                ([version]$($vckey.$i_vcval) -lt [version]$($compverkey[1]))) {
                  $o_AVStatus = "Up-to-Date : $($false) (REG Check)`r`n"
              }
              $o_AVStatus += "Core Version : $($o_AVVersion) - Expected : '$($compverkey[0])'`r`n"
              $o_AVStatus += "VC Version : $($vckey.$i_vcval) - Expected : '$($compverkey[1])'`r`n"
            } catch {
          ...
          $o_AVStatus += "Real-Time Scanning : $($o_RTstate)"
          ...
          $o_AVStatus += "Tamper Protection : $($tamper)"
          ...
          $o_DefStatus = "Definition Status : $($o_DefStatus)"
