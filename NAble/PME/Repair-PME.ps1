...
write-output "Repair-PME $Version $VersionDate"
write-output "-------------------------------"
...
Function Confirm-Elevation {
    ...
    Else {
        write-output "OK: Script is running as administrator"
    }
}
...
Function Get-RepairPMEUpdate {
    If ($UpdateCheck -eq "Yes") {
        write-output "Checking if update is available for Repair-PME script..."    
        ...
        If ([version]$Version -ge [version]$LatestPMEVersion) {
            write-output "OK: Repair-PME is up to date"
        }
...
Function Test-Connectivity {
    ...
    If (($PSVersionTable.PSVersion -ge "4.0") -and (!($OSVersion -match 'Windows 7')) -and (!($OSVersion -match '2008 R2'))) {
        ...
        $List1 | ForEach-Object {
            ...
            If ($Test1.tcptestsucceeded -eq $True) {
                write-output "OK: Connectivity to https://$_ ($(($Test1).RemoteAddress.IpAddressToString)) established"
                $HTTPSError += "No"
            }
            Else {
                write-output "ERROR: Unable to establish connectivity to https://$_ ($(($Test1).RemoteAddress.IpAddressToString))"
                $HTTPSError += "Yes"
            }
        }
        ...
        $List2 | ForEach-Object {
            ...
            If ($Test1.tcptestsucceeded -eq $True) {
                write-output "OK: Connectivity to http://$_ ($(($Test1).RemoteAddress.IpAddressToString)) established"
                $HTTPError += "No"
            }
            Else {
                write-output "ERROR: Unable to establish connectivity to http://$_ ($(($Test1).RemoteAddress.IpAddressToString))"
                $HTTPError += "Yes"
            }
        }
    ...
    Else {
        write-output "Performing HTTPS connectivity tests for PME required destinations using legacy method..."
        $List1 = @("sis.n-able.com")
        $HTTPSError = @()
        $List1 | ForEach-Object {
            $Test1 = Test-Port $_ 443
            If ($Test1 -eq $True) {
                write-output "OK: Connectivity to https://$_ established"
                $HTTPSError += "No"
            }
            Else {
                write-output "ERROR: Unable to establish connectivity to https://$_ established"
                $HTTPSError += "Yes"
            }
        }

        write-output "Performing HTTP connectivity tests for PME required destinations using legacy method..."
        $HTTPError = @()
        $List2 = @("sis.n-able.com", "download.windowsupdate.com", "fg.ds.b1.download.windowsupdate.com")
        $List2 | ForEach-Object {
            $Test1 = Test-Port $_ 80
            If ($Test1 -eq $True) {
                write-output "OK: Connectivity to http://$_ established"
                $HTTPError += "No"
            }
            Else {
                write-output "ERROR: Unable to establish connectivity to http://$_ established"
                $HTTPError += "Yes"
            }
        }
...
Function Test-NableCertificate {
    If ($null -eq $Fallback) {
        write-output "Downloading and checking certificate chain for sis.n-able.com..."
        ...
        $CertificateChain | ForEach-Object {
            ...
            Else {
                write-output "OK: Certificate for ($($_.Subject)) is valid" 
            }
        }
    }
}
...
Function Get-NCAgentVersion {
    ...
    write-output "Checking if N-Central Agent is already installed..."
    $IsNCAgentInstalled = ""
        ...
        If ($null -ne $installed) {
            ForEach ($app in $installed) {
                If ($($app.DisplayName) -eq "Windows Agent" -and $($app.Publisher) -eq "N-able Technologies") {
                    ...
                    write-output "N-Central Agent Installed: Yes"
                    ...
                    If ($($app.DisplayVersion) -ge "12.2.0.274") {
                        write-output "N-Central Agent PME Compatible: Yes"
                    } 
                    Else {
                        write-output "N-Central Agent PME Compatible: No"
                ...
                Else {
                    ...
                    write-output "N-Central Agent Installed: No"
...
Function Confirm-PMEInstalled {
    write-output "Checking if PME Components are already installed..."
            ...
            If ($null -ne $installed) {
                ForEach ($app in $installed) {
                    ...
                    If ($($app.DisplayName) -eq "Patch Management Service Controller") {
                        ...
                        $IsPMEAgentInstalled = "Yes"
                        write-output "PME Patch Management Service Controller Already Installed: Yes"
    ...
    If ($IsPMEAgentInstalled -ne "Yes") {
        write-output "PME Agent / Patch Management Service Controller Already Installed: No"
    }
            ...
            If ($null -ne $installed) {
                ForEach ($app in $installed) {
                    ...
                    If ($($app.DisplayName) -eq "Request Handler Agent") {
                        ...
                        $IsPMERPCServerServiceInstalled = "Yes"
                        write-output "PME Request Handler Agent Already Installed: Yes"
    ...
    If ($IsPMERPCServerServiceInstalled -ne "Yes") {
        write-output "PME RPC Server Service / Request Handler Agent Already Installed: No"
    }
            ...
            If ($null -ne $installed) {
                ForEach ($app in $installed) {
                    ...
                    If ($($app.DisplayName) -eq "File Cache Service Agent") {
                        ...
                        $IsPMECacheServiceInstalled = "Yes"
                        write-output "PME File Cache Service Agent Already Installed: Yes"
    ...
    If ($IsPMECacheServiceInstalled -ne "Yes") {
        write-output "PME Cache Service / File Cache Service Agent Already Installed: No"
    }
}
...
Function Get-PMESetupDetails {
    ...
    write-output "Checking Latest PME version..."
...
Function Confirm-PMERecentInstall {
    If (($IsPMEAgentInstalled -eq "Yes") -or ($IsPMERPCServerServiceInstalled -eq "Yes") -or ($IsPMECacheServiceInstalled -eq "Yes")) {
        ...
        write-output "INFO: Repair-PME will force repair without update pending check if PME was installed in the last ($ForceRepairRecentInstallDays) days" -BackgroundColor Black
...
Function Confirm-PMEUpdatePending {
    ...
    If (($IsPMEAgentInstalled -eq "Yes") -and ($BypassUpdatePendingCheck -eq "No")) {
        ...
        write-output "Checking if PME update pending..."
        write-output "INFO: Script will proceed ($RepairAfterUpdateDays) days after a new version of PME has been released" -BackgroundColor Black
...
Function Clear-RepairPME {
    ...
    write-output "Repair-PME Log Cleanup..."
...
Function Invoke-PMEDiagnostics {
    ...
    write-output "Checking Diagnostics..."
...
Function Stop-PMESetup {
    ...
    write-output "Stopping any running instances of PME install/uninstall..."
    ...
    ForEach ($Process in $Processes) {
        ...
        Else {
            write-output "OK: $Process is not currently running"
        }
    }
}

Function Stop-PMEServices {
    write-output "Stopping PME Services..."
    ...
    ForEach ($Service in $Services) {
        ...
        If (($ServiceStatus -eq "Running") -or ($ServiceStatus -eq "Stopping") -or ($ServiceStatus -eq "Suspended")) {
            ...
            If ($ServiceStatus -eq "Stopped") {
                write-output "OK: $Service service successfully stopped"
            }
            Else {
        ...
        Else {
            write-output "OK: $Service is not running"
        }
    }
}
...
Function Clear-PME {
    ...
    write-output "PME Cache Cleanup..."
...
Function Get-PMEConfigMisconfigurations {
    ...
    write-output "Checking PME Configuration..."
    Try {
            ...
            $CacheServiceConfig = $xml.Configuration
                If ($null -ne $CacheServiceConfig) {
                    ...
                    ElseIf ($CacheServiceConfig.CanBypassProxyCacheService -eq "True") {
                        write-output "INFO: Patch profile allows PME to fallback to external sources" -BackgroundColor Black
                    }
                    Else {
                    ...
                    If ($CacheServiceConfig.CacheSizeInMB -eq 10240) {
                        write-output "INFO: Cache Service is set to default cache size of 10240 MB" -BackgroundColor Black
                    }
                    Else {
...
Function Set-PMEConfig {
    ...
    write-output "Setting PME Configuration..."
}

Function Install-PME {
    write-output "Install PME..."
    ...
    If (Test-Path -Path "$PMEArchives\PMESetup_$($PMEDetails.Version).exe") {
        ...
        If ($Download -eq $($PMEDetails.SHA256Checksum)) {
            ...
            If ($Install.ExitCode -eq 0) {
                write-output "OK: $($PMEDetails.Name) version $($PMEDetails.Version) successfully installed"
...
Function Confirm-PMEServices {
    If ($Install.ExitCode -eq 0) {
        write-output "Checking PME services post-installation..."
        ...
        If (($PMEAgentStatus -eq 'Running') -and ($FileCacheServiceAgentStatus -eq 'Running') -and ($RequestHandlerAgentStatus -eq 'Running')) {
            write-output "OK: All PME services are installed and running following installation"
        }
        Else {
