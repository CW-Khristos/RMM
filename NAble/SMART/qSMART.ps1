...
#REGION ----- FUNCTIONS ----
  ...
  function mapSMART($varID,$varVAL) {
    ...
    write-host -ForegroundColor green $arrDRV[$i].drvID "     " $varID "     " $varVAL
...
foreach ($objDRV in $arrDRV) {
  ...
  if ($objDRV.drvID-eq $i_drive) {
    ...
    $allout += "  - Critical Comp. Temp Time (NVMe) : $($arrDRV[$i].nvmecctemp)"
    write-host $allout -foregroundcolor $ccode
    #NABLE RMM OUTPUT
    $o_fail = $arrDRV[$i].fail
    #HDD ATTRIBUTES
    $o_reallocated = $arrDRV[$i].id5
    $o_spinretry = $arrDrv[$i].id10
    $o_enderror = $arrDRV[$i].id184
    $o_uncorrectable = $arrDRV[$i].id187
    $o_command = $arrDRV[$i].id188
    $o_airtemp = $arrDRV[$i].id190
    $o_temperature = $arrDRV[$i].id194
    $o_reallocation = $arrDRV[$i].id196
    $o_pending = $arrDRV[$i].id197
    $o_offuncorrectable = $arrDRV[$i].id198
    $o_softread = $arrDRV[$i].id201
    #SSD ATTRIBUTES
    $o_availspace = $arrDRV[$i].id170
    $o_programfail = $arrDRV[$i].id171
    $o_erasefail = $arrDRV[$i].id172
    $o_wearlevel = $arrDRV[$i].id173
    $o_erasefail2 = $arrDRV[$i].id176
    $o_wearlevel2 = $arrDRV[$i].id177
    $o_programfail2 = $arrDRV[$i].id181
    $o_erasefail3 = $arrDRV[$i].id182
    $o_drivelife = $arrDRV[$i].id230
    $o_ssdlife = $arrDRV[$i].id231
    $o_endurance = $arrDRV[$i].id232
    #NVME ATRIBUTES
    $o_nvmewarn = $arrDRV[$i].nvmewarn
    $o_nvmetemp = $arrDRV[$i].nvmetemp
    $o_nvmeavail = $arrDRV[$i].nvmeavail
    $o_nvmemdi = $arrDRV[$i].nvmemdi
    $o_nvmeerr = $arrDRV[$i].nvmeerr
    $o_nvmewctemp = $arrDRV[$i].nvmewctemp
    $o_nvmecctemp = $arrDRV[$i].nvmecctemp
  }
}
#END SCRIPT
#------------