...
write-output " - UPDATING SMARTCTL"
...
write-output " - UPDATING SMARTCTL DRIVE DATABASE"
$output = Get-ProcessOutput -FileName $dbEXE -Args "/S"
#write-output $output
#POPULATE DRIVES
write-output " - ENUMERATING CONNECTED DRIVES"
$script:arrDRV = @()
...
foreach ($objDRV in $arrDRV) {
  ...
  if ($objDRV.drvID-eq $i_drive) {
    write-output " - QUERYING DRIVE : $($objDRV.drvID)"
