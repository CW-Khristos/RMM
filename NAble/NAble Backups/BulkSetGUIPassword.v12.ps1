...
#region ----- Backup.Management JSON Calls ----
  ...
  Function UpdateCustomColumnA($DeviceId,$ColumnId,$Message) {
    ...
    write-output $Script:strLineSeparator
    write-output "  UpdateA : $($Script:updateCC)"
  ...
  Function UpdateCustomColumnB($DeviceId,$ColumnId,$Message) {
    ...
    write-output $Script:strLineSeparator
    write-output "  UpdateB : $($Script:updateCC)"
  ...
  Function UpdateCustomColumnC($DeviceId,$ColumnId,$Message) {
    ...
    if ($ModifyAccountSessionErrorCode) {
      ...
      write-output $Script:strLineSeparator
      write-output "  DEVICE | $($DeviceId) | $($selecteddevice.DeviceName) | ASSIGN GUI PW ERROR | $ModifyAccountSessionErrorMsg"
      # (Exit Script if there is a problem)
      #Break Script
    } elseif (-not $ModifyAccountSessionErrorCode) {
      # (No error)
      write-output $Script:strLineSeparator
      write-output "  SUCCESS UPDATING GUI PW COLUMN"
    }
    write-output $Script:strLineSeparator
    write-output "  UpdateC : $ModifyAccountSessionErrorMsg"
...
if($null -eq $SelectedDevices) {
  ...
  foreach ($selecteddevice in $SelectedDevices) {
    ...
    write-output $Script:strLineSeparator
    write-output "  Updating GUI PW Column for $device - " $selecteddevice.AccountID 2048 $password
