<# ----- About: ----
    ...
    # Modifications : Chris Bledsoe - cbledsoe@ipmcomputers.com
# -----------------------------------------------------------#>  ## About
...
#[CmdletBinding()]
#  Param (
#    [Parameter(Mandatory=$False)] [ValidateRange(1,84)] [int]$Months=48,        ## Clean Archives older than X number of months
...
#region ----- Environment, Variables, Names and Paths ----
  ...
  $Months = [int]$env:intMonths
  $Force = $env:blnForce
  if ($Force.tolower() -eq "true") {
    $Force = $true
  } elseif ($Force.tolower() -eq "false") {
    $Force = $false
  }
  $CheckArchive = $env:blnCheckArchive
  if ($CheckArchive.tolower() -eq "true") {
    $CheckArchive = $true
  } elseif ($CheckArchive.tolower() -eq "false") {
    $CheckArchive = $false
  }
...
#region ----- Functions ----
  if (Get-Module -ListAvailable -Name NtpTime) {
      write-output "  Module NtpTime Already Installed"
  } else {
    try {
  ...
  Function Get-FPvisa {
    ...
    if ((get-process "BackupFP" -ea SilentlyContinue) -eq $Null) { 
      ...
      try {
        $ErrorActionPreference = 'Stop'; $UIToken = & $clienttool in-agent-authentication-token.get -config-path $config | convertfrom-json 
      } catch { 
  ...
  Function GetArchiveSettings {
    ...
    if (($CheckArchive) -or ($AddArchive)) {
      ...
      Write-Output "  $(Get-TimeStamp) Getting current archive settings for $($ENV:COMPUTERNAME) | $($DeviceName.value)" | 
  ...
  Function ClienttoolGetArchiveSessions {
    ...
    $Inventory = $Sessions | Where-Object {(($_.FLAGS -eq'A---?') -or ($_.FLAGS -eq 'A---?---?')) -and ($_.TyPE -eq "Backup")} | 
      Select-Object * | Sort-Object START
    ...
    $Script:CleanSessions = $Sessions | Where-Object {(($_.FLAGS -eq'A---?') -or ($_.FLAGS -eq 'A---?---?')) -and 
      ($_.TyPE -eq "Backup") -and ([datetime]$_.START -lt $filterDate)} | 
        Select-Object -Property START,DSRC,STATE,TYPE,FLAGS,SELS | 
          Sort-Object START 
  ...
  Function XMLLookupArchiveSessionIds {
    ...
    #$XmlDocument.SessionStatistics.Session | Where-Object {($_.Archived -eq "0")} | 
    #  Select-Object -Property Id,Plugin, StartTimeUTC,Status,Type,Archived | 
    ...
    #$XmlDocument.SessionStatistics.Session | Where-Object {($_.Archived -eq "1") -and ([datetime]$_.StartTimeUTC -lt $filterDate)} | 
    #  Select-Object -Property Id,Plugin, StartTimeUTC,Status,Type,Archived | 
    #    Sort-Object StartTime | format-table

    $Script:CleanArchive = $XmlDocument.SessionStatistics.Session | Where-Object {($_.Archived -eq "1")} | 
      Select-Object -Property @{Name="StartTime";Expression={$_.StartTimeUTC}},Id,Plugin,Status,Type,Archived | 
        Sort-Object StartTime 
    $Script:CleanArchive | ForEach-Object {$_.StartTime = get-localtime ([datetime]$_.StartTime)} | 
      Where-Object {([datetime]$_.StartTime -lt $filterDate)} | 
        Select-Object -Property Id,Plugin,StartTime,Status,Type,Archived | 
          Sort-Object StartTime | format-table
    #$Script:CleanArchive | Where-Object {([datetime]$_.StartTime -lt $filterDate)} | 
    #  Select-Object -Property Id,Plugin,StartTime,Status,Type,Archived | 
    #    Sort-Object StartTime |format-table
  ...
  Function Clean-Archives {
    ...
    if ($CleanArchives.error) {
      $CleanArchives.error.message
    } else {
      #Debug $data.method
      #Debug $data.params
...
#region ----- Body ----
  ...
  if ($months -lt 1) { 
    Write-Warning "Archive Retention Expiration of < 1 Months Is Not Allowed"; Break
  } else {
  ...
  $ArchiveSessionsToClean | Select-Object -Property ID,DataSource,StartTime,State,Type,SelectedSize,Flags | 
    Sort-Object StartTime | format-table

  Write-Output "  $(Get-TimeStamp) Found $($ArchiveSessionsToClean.ID.count) archive session(s) older than $Filterdate" | 
    Out-file $ScriptLog -append
  $ArchiveSessionsToClean | Select-Object -Property ID,DataSource,StartTime,State,Type,SelectedSize,Flags | 
    Sort-Object StartTime | format-table | Out-file $ScriptLog -append
  ...
  #If ($force -ne $true) {
  #  $strAreYouSure = Read-Host -Prompt "Do you want to clean all Archive Sessions prior to [$Filterdate]? This can not be undone! Type (Y/N)"
  #} 
  ...
  If (($force) -or ($strAreYouSure -eq "Y")) {
    ...
    If ($ArchiveSessionIDsToClean -eq $null) { 
      ...
      Write-Output "  $(Get-TimeStamp) Removed $($ArchiveSessionsToClean.ID.count) archive session(s) older than $Filterdate`n" | 
        Out-file $ScriptLog -append
    }
  } else {
    Write-Output $Script:strLineSeparator
