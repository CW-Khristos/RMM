  ...
  function logERR ($intSTG, $strModule, $strErr) {
    ...
    switch ($intSTG) {
      1 {                                                         #'ERRRET'=1 - NOT ENOUGH ARGUMENTS, END SCRIPT
        ...
        write-output "$($strLineSeparator)`r`n$($(get-date)) - MSPBackup_Monitor - NO ARGUMENTS PASSED, END SCRIPT`r`n`r`n"
      }
      2 {                                                         #'ERRRET'=2 - END SCRIPT
        ...
        write-output "$($strLineSeparator)`r`n$($(get-date)) - MSPBackup_Monitor - ($($strModule)) :"
        write-output "$($strLineSeparator)`r`n`t$($strErr)`r`n`tEND SCRIPT`r`n$($strLineSeparator)"
      }
      3 {                                                         #'ERRRET'=3
        $script:blnWARN = $false
        $script:diag += "`r`n$($strLineSeparator)`r`n$($(get-date)) - MSPBackup_Monitor - $($strModule) :"
