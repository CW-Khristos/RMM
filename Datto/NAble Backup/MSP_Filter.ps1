#REGION ----- DECLARATIONS ----
  ...
  Param (
    ...
    [Parameter(Mandatory=$true)]$strURL
  ...
  function logERR ($intSTG) {
    ...
    switch ($intSTG) {
      0 {                                                         #'ERRRET'=0 - CLIENTTOOL CHECK PASSED
        ...
        write-output "$((Get-Date).ToString('dd-MM-yyyy hh:mm:ss'))`t - MSP_FILTER - CLIENTTOOL CHECK PASSED"
      }
      1 {                                                         #'ERRRET'=1 - CLIENTTOOL.EXE NOT PRESENT, END SCRIPT
        $script:blnFAIL = $true
        $script:diag += "$((Get-Date).ToString('dd-MM-yyyy hh:mm:ss'))`t - MSP_FILTER - CLIENTTOOL.EXE NOT PRESENT, END SCRIPT`r`n`r`n"
        write-output "$((Get-Date).ToString('dd-MM-yyyy hh:mm:ss'))`t - MSP_FILTER - CLIENTTOOL.EXE NOT PRESENT, END SCRIPT`r`n"
      }
      2 {                                                         #'ERRRET'=2 - NOT ENOUGH ARGUMENTS, END SCRIPT
        ...
        write-output "$((Get-Date).ToString('dd-MM-yyyy hh:mm:ss'))`t - MSP_FILTER - NO ARGUMENTS PASSED, END SCRIPT`r`n"
      }
    }
  }
    ...
    if (($null -ne $strSFOL) -and ($strSFOL -ne "")) {
      ...
      foreach ($pFOL in $arrPFOL) {
        ...
        if (($null -ne $pFOL) -and ($pFOL -ne "")) {
          ...
          if ($strSFOL.tolower() -match $pFOL.tolower()) {
            ...
            $objSFOL = (get-item -path $strSFOL -force)
            if ($objSFOL.PSisContainer) {
                write-output "$($objSFOL.fullname) is a directory"
                (get-item -path $objSFOL.fullname -force).attributes = "Directory"
            } elseif (-not ($objSFOL.PSisContainer)) {
                write-output "$($objSFOL.fullname) is a file"
                (get-item -path $objSFOL.fullname -force).attributes = "Archive"
            }
            ...
            write-output "`t`t - EXECUTING : $($cliPath) control.selection.modify -datasource FileSystem -include `"$($strSFOL)`""
            ...
            #RE-APPLY ORIGINAL ATTRIBUTES
            (get-item -path $objSFOL.fullname -force).attributes = $objSFOL.attributes
      ...
      if (-not $blnFND) {
        ...
        if ($strSFOL -like '`*') {
          ...
          foreach ($subFIL in $colSFIL) {
            if ($subFIL.fullname.tolower() -match $strSFOL.split("*")[0].tolower()) {
              if ($subFIL.fullname.tolower() -match $strSFOL.split("*")[1].tolower()) {
                ...
                #CAPTURE ORIGINAL ATTRIBUTES
                $objSFOL = (get-item -path $subFIL.fullname -force)
                if ($objSFOL.PSisContainer) {
                    write-output "$($objSFOL.fullname) is a directory"
                    (get-item -path $objSFOL.fullname -force).attributes = "Directory"
                } elseif (-not ($objSFOL.PSisContainer)) {
                ...
                write-output "`t`t - EXECUTING : $($cliPath) control.selection.modify -datasource FileSystem -exclude `"$($subFIL.fullname)`""
          ...
          foreach ($subSFOL in $colSFOL) {
            if ($subSFOL.fullname -match "Profile ") {
              foreach ($item in $arrPROF) {
                #CAPTURE ORIGINAL ATTRIBUTES
                $objSFOL = (get-item -path "$($subSFOL.fullname)\$($item)" -force)
                if ($objSFOL.PSisContainer) {
                    write-output "$($objSFOL.fullname) is a directory"
                    (get-item -path $objSFOL.fullname -force).attributes = "Directory"
                } elseif (-not ($objSFOL.PSisContainer)) {
                ...
                write-output "`t`t - EXECUTING : $($cliPath) control.selection.modify -datasource FileSystem -exclude `"$($subSFOL.fullname)\$($item)`""
          ...
          write-output "`t`t - EXECUTING : $($cliPath) control.selection.modify -datasource FileSystem -exclude `"$($strSFOL)`""
...
write-output "$((Get-Date).ToString('dd-MM-yyyy hh:mm:ss')) - EXECUTING MSP_FILTER"
...
if (-not ($script:blnFAIL)) {
  switch ($strOPT.tolower()) {
        ...
        foreach ($line in $psTXT) {
          if (($null -ne $line) -and ($line -ne "")) {
            ...
            if ($line -match "\*") {                                #APPLY BACKUP FILTERS
              ...
              write-output "`t`t - EXECUTING : $($cliPath) control.filter.modify -add `"$($strPATH)`""
              ...
              #CAPTURE ORIGINAL ATTRIBUTES
              $objSFOL = (get-item -path $strPATH -force)
              if ($objSFOL.PSisContainer) {
                  write-output "$($objSFOL.fullname) is a directory"
                  (get-item -path $objSFOL.fullname -force).attributes = "Directory"
              } elseif (-not ($objSFOL.PSisContainer)) {
              ...
              write-output "`t`t - EXECUTING : $($cliPath) control.selection.modify -datasource FileSystem -exclude `"$($strPATH)`""
      ...
      if ($script:blnPSTXT) {
        foreach ($line in $psTXT) {
          if (($null -ne $line) -and ($line -ne "")) {
            ...
            write-output "`t`t - EXECUTING : $($cliPath) control.selection.modify -datasource FileSystem -include `"$($strPATH)`""
    ...
    "cloud" {
      ...
      write-output "`t - RESETTING CURRENT MSP BACKUP INCLUDES"
  ...
  write-output "`r`n`t - PERFORMING FINAL EXCLUDES"
  #PROCEED WITH EXCLUDING DEFAULTS
  for ($intEXCL = 65; $intEXCL -le 90; $intEXCL++) {
    #CAPTURE ORIGINAL ATTRIBUTES
    $objSFOL = (get-item -path "$([char]$($intEXCL)):\Temp" -force)
    if ($objSFOL.PSisContainer) {
        write-output "$($objSFOL.fullname) is a directory"
        (get-item -path $objSFOL.fullname -force).attributes = "Directory"
    } elseif (-not ($objSFOL.PSisContainer)) {
    ...
    write-output "`t`t - EXECUTING : $($cliPath) control.selection.modify -datasource FileSystem -exclude `"$([char]$($intEXCL)):\Temp`""
    ...
    #CAPTURE ORIGINAL ATTRIBUTES
    $objSFOL = (get-item -path "$([char]$($intEXCL)):\Recovery" -force)
    if ($objSFOL.PSisContainer) {
        write-output "$($objSFOL.fullname) is a directory"
        (get-item -path $objSFOL.fullname -force).attributes = "Directory"
    } elseif (-not ($objSFOL.PSisContainer)) {
    ...
    write-output "`t`t - EXECUTING : $($cliPath) control.selection.modify -datasource FileSystem -exclude `"$([char]$($intEXCL)):\Recovery`""
    ...
    start-sleep -milliseconds 20
    #CAPTURE ORIGINAL ATTRIBUTES
    $objSFOL = (get-item -path "$([char]$($intEXCL)):\RECYCLED" -force)
    if ($objSFOL.PSisContainer) {
        write-output "$($objSFOL.fullname) is a directory"
        (get-item -path $objSFOL.fullname -force).attributes = "Directory"
    } elseif (-not ($objSFOL.PSisContainer)) {
    ...
    write-output "`t`t - EXECUTING : $($cliPath) control.selection.modify -datasource FileSystem -exclude `"$([char]$($intEXCL)):\RECYCLED`""
    ...
    start-sleep -milliseconds 20
    #CAPTURE ORIGINAL ATTRIBUTES
    $objSFOL = (get-item -path "$([char]$($intEXCL)):\AV_ASW" -force)
    if ($objSFOL.PSisContainer) {
        write-output "$($objSFOL.fullname) is a directory"
        (get-item -path $objSFOL.fullname -force).attributes = "Directory"
    } elseif (-not ($objSFOL.PSisContainer)) {
    ...
    write-output "`t`t - EXECUTING : $($cliPath) control.selection.modify -datasource FileSystem -exclude `"$([char]$($intEXCL)):\AV_ASW`""
    ...
    start-sleep -milliseconds 20
    #CAPTURE ORIGINAL ATTRIBUTES
    $objSFOL = (get-item -path "$([char]$($intEXCL)):\GetCurrent" -force)
    if ($objSFOL.PSisContainer) {
        write-output "$($objSFOL.fullname) is a directory"
        (get-item -path $objSFOL.fullname -force).attributes = "Directory"
    } elseif (-not ($objSFOL.PSisContainer)) {
    ...
    write-output "`t`t - EXECUTING : $($cliPath) control.selection.modify -datasource FileSystem -exclude `"$([char]$($intEXCL)):\GetCurrent`""
    ...
    start-sleep -milliseconds 20
    #CAPTURE ORIGINAL ATTRIBUTES
    $objSFOL = (get-item -path "$([char]$($intEXCL)):\$Recycle.Bin" -force)
    if ($objSFOL.PSisContainer) {
        write-output "$($objSFOL.fullname) is a directory"
        (get-item -path $objSFOL.fullname -force).attributes = "Directory"
    } elseif (-not ($objSFOL.PSisContainer)) {
    ...
    write-output "`t`t - EXECUTING : $($cliPath) control.selection.modify -datasource FileSystem -exclude `"$([char]$($intEXCL)):\$Recycle.Bin`""
    ...
    start-sleep -milliseconds 20
    #CAPTURE ORIGINAL ATTRIBUTES
    $objSFOL = (get-item -path "$([char]$($intEXCL)):\$Windows.~BT" -force)
    if ($objSFOL.PSisContainer) {
        write-output "$($objSFOL.fullname) is a directory"
        (get-item -path $objSFOL.fullname -force).attributes = "Directory"
    } elseif (-not ($objSFOL.PSisContainer)) {
    ...
    write-output "`t`t - EXECUTING : $($cliPath) control.selection.modify -datasource FileSystem -exclude `"$([char]$($intEXCL)):\$Windows.~BT`""
    ...
    start-sleep -milliseconds 20
    #CAPTURE ORIGINAL ATTRIBUTES
    $objSFOL = (get-item -path "$([char]$($intEXCL)):\$Windows.~WS" -force)
    if ($objSFOL.PSisContainer) {
        write-output "$($objSFOL.fullname) is a directory"
        (get-item -path $objSFOL.fullname -force).attributes = "Directory"
    } elseif (-not ($objSFOL.PSisContainer)) {
    ...
    write-output "`t`t - EXECUTING : $($cliPath) control.selection.modify -datasource FileSystem -exclude `"$([char]$($intEXCL)):\$Windows.~WS`""
    ...
    start-sleep -milliseconds 20
    #CAPTURE ORIGINAL ATTRIBUTES
    $objSFOL = (get-item -path "$([char]$($intEXCL)):\Windows10Upgrade" -force)
    if ($objSFOL.PSisContainer) {
        write-output "$($objSFOL.fullname) is a directory"
        (get-item -path $objSFOL.fullname -force).attributes = "Directory"
    } elseif (-not ($objSFOL.PSisContainer)) {
    ...
    write-output "`t`t - EXECUTING : $($cliPath) control.selection.modify -datasource FileSystem -exclude `"$([char]$($intEXCL)):\Windows10Upgrade`""
    ...
    start-sleep -milliseconds 20
    #CAPTURE ORIGINAL ATTRIBUTES
    $objSFOL = (get-item -path "$([char]$($intEXCL)):\hiberfil.sys" -force)
    if ($objSFOL.PSisContainer) {
        write-output "$($objSFOL.fullname) is a directory"
        (get-item -path $objSFOL.fullname -force).attributes = "Directory"
    } elseif (-not ($objSFOL.PSisContainer)) {
    ...
    write-output "`t`t - EXECUTING : $($cliPath) control.selection.modify -datasource FileSystem -exclude `"$([char]$($intEXCL)):\hiberfil.sys`""
    ...
    start-sleep -milliseconds 20
    #CAPTURE ORIGINAL ATTRIBUTES
    $objSFOL = (get-item -path "$([char]$($intEXCL)):\pagefile.sys" -force)
    if ($objSFOL.PSisContainer) {
        write-output "$($objSFOL.fullname) is a directory"
        (get-item -path $objSFOL.fullname -force).attributes = "Directory"
    } elseif (-not ($objSFOL.PSisContainer)) {
    ...
    write-output "`t`t - EXECUTING : $($cliPath) control.selection.modify -datasource FileSystem -exclude `"$([char]$($intEXCL)):\pagefile.sys`""
    ...
    start-sleep -milliseconds 20
    #CAPTURE ORIGINAL ATTRIBUTES
    $objSFOL = (get-item -path "$([char]$($intEXCL)):\swapfile.sys" -force)
    if ($objSFOL.PSisContainer) {
        write-output "$($objSFOL.fullname) is a directory"
        (get-item -path $objSFOL.fullname -force).attributes = "Directory"
    } elseif (-not ($objSFOL.PSisContainer)) {
    ...
    write-output "`t`t - EXECUTING : $($cliPath) control.selection.modify -datasource FileSystem -exclude `"$([char]$($intEXCL)):\swapfile.sys`""
    ...
    start-sleep -milliseconds 20
    #CAPTURE ORIGINAL ATTRIBUTES
    $objSFOL = (get-item -path "$([char]$($intEXCL)):\System Volume Information" -force)
    if ($objSFOL.PSisContainer) {
        write-output "$($objSFOL.fullname) is a directory"
        (get-item -path $objSFOL.fullname -force).attributes = "Directory"
    } elseif (-not ($objSFOL.PSisContainer)) {
    ...
    write-output "`t`t - EXECUTING : $($cliPath) control.selection.modify -datasource FileSystem -exclude `"$([char]$($intEXCL)):\System Volume Information`""
  ...
  write-output "`r`n`t - CHECKING USER FOLDERS"
      ...
      for ($intCOL = 0; $intCOL -le $arrEXCL.length; $intCOL++) {
        ...
        if (($null -ne $arrEXCL[$intCOL]) -and ($arrEXCL[$intCOL] -ne "")) {
          ...
          if ($subFOL.tolower() -match $arrEXCL[$intCOL].tolower()) {
            ...
            write-output "`t`t - UNNEEDED / TO EXCLUDE USER : $($arrEXCL[$intCOL])"
            #MARK 'UNNEEDED / TO EXCLUDE'
            $blnFND = $true
            #DISABLED TO PREVENT OVER-WRITE OF TECHNICIAN SELECTIONS AT A LATER TIME
            #PROCEED WITH INCLUDING ENTIRE USER DIRECTORY
            ...
            foreach ($subUFOL in $objUFOL) {
              #CAPTURE ORIGINAL ATTRIBUTES
              $objSFOL = (get-item -path $subUFOL.fullname -force)
              if ($objSFOL.PSisContainer) {
                  write-output "$($objSFOL.fullname) is a directory"
                  (get-item -path $objSFOL.fullname -force).attributes = "Directory"
              } elseif (-not ($objSFOL.PSisContainer)) {
              ...
              write-output "`t`t - EXECUTING : $($cliPath) control.selection.modify -datasource FileSystem -exclude `"$($subUFOL.fullname)`""
              ...
              write-output "`t`t - EXECUTING : $($cliPath) control.selection.modify -datasource FileSystem -include `"$($subUFOL.fullname)\desktop.ini`""
      ...
      if (-not ($blnFND)) {
        ...
        for ($intPCOL = 0; $intPCOL -le $arrPUSR.length; $intPCOL++) {
          ...
          if (($null -ne $arrPUSR[$intPCOL]) -and ($arrPUSR[$intPCOL] -ne "")) {
            ...
            if ($subFOL -match $arrPUSR[$intPCOL]) {
              ...
              write-output "`t`t - PROTECTED : $($arrPUSR[$intPCOL])"
              #CAPTURE ORIGINAL ATTRIBUTES
              $objSFOL = (get-item -path $($subFOL.fullname) -force)
              if ($objSFOL.PSisContainer) {
                  write-output "$($objSFOL.fullname) is a directory"
                  (get-item -path $objSFOL.fullname -force).attributes = "Directory"
              } elseif (-not ($objSFOL.PSisContainer)) {
              ...
              #PROCEED WITH INCLUDING ENTIRE USER DIRECTORY
              $script:diag += "`t`t - EXECUTING : $($cliPath) control.selection.modify -datasource FileSystem -include `"$($subFOL)`"`r`n"
              write-output "`t`t - EXECUTING : $($cliPath) control.selection.modify -datasource FileSystem -include `"$($subFOL)`""
              $output = Get-ProcessOutput -filename "$($cliPath)" -args "control.selection.modify -datasource FileSystem -include `"$($subFOL)`""
              $script:diag += "`t`t`t - StdOut : $($output.standardoutput) - StdErr : $($output.standarderror)`r`n"
              write-output "`t`t`t - StdOut : $($output.standardoutput) - StdErr : $($output.standarderror)"
              #RE-APPLY ORIGINAL ATTRIBUTES
              (get-item -path $objSFOL.fullname -force).attributes = $objSFOL.attributes
        ...
        if (-not ($blnFND)) {
          ...
          if (test-path -path "$($subFOL)") {
            ...
            write-output "`t`t - ENUMERATING : $($subFOL)"
            #DISABLED TO PREVENT OVER-WRITE OF TECHNICIAN SELECTIONS AT A LATER TIME
            #PROCEED WITH INCLUDING ENTIRE USER DIRECTORY
            #objOUT.write vbnewline & now & vbtab & vbtab & _
            #  "EXECUTING : $($cliPath) control.selection.modify -datasource FileSystem -include " & chr(34) & strFOL & chr(34)
...
write-output "$((Get-Date).ToString('dd-MM-yyyy hh:mm:ss')) - MSP_FILTER COMPLETE"
#Stop script execution time calculation
StopClock
#WRITE LOGFILE
...
#DATTO OUTPUT
if ($script:blnWARN) {
  write-DRMMAlert "MSP_FILTER : Execution Failure : See Diagnostics"
  write-DRMMDiag "$($script:diag)"
  exit 1
} elseif (-not $script:blnWARN) {
  write-DRMMAlert "MSP_FILTER : Completed Execution"
  write-DRMMDiag "$($script:diag)"
  exit 0
}
#END SCRIPT
