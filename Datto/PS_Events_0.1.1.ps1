...
if ($global:cmds -eq 0) {
  ...
  foreach ($cmd in $global:hashCMD.Keys) {
    $global:diag += "`r`n`r`nTriggered Command : $($global:hashCMD[$cmd].TriggeredCommand)`r`nTimestamp : $($global:hashCMD[$cmd].TimeCreated)`r`nDetails : $($global:hashCMD[$cmd].EventMessage)"
  }
