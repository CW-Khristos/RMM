...
$o_PwdNoExpire = $null
$o_SIDHistory = $null
$o_RevEncryptUser = $null
$o_PwdNoRequire = $null
$o_KerbUser = $null
$o_KerbPreAuthUser = $null
...
#---------------------------------------------------------------------------------------------------------------------------------------------
  ...
  Function Get-PrivilegedGroupChanges {
    ...
    if ($Members -ne $Null) {
      ...
      $o_Notes += "`r`nGet-PrivilegedGroupChanges : Could not obtain AD Replication data: 'Get-ADReplicationAttributeMetadata'."
    }
...
try {
 Import-Module ActiveDirectory   
  ...
  $o_Notes += "`r`nFailed Importing Active Directory Module..!"
  $blnWARN = $true
  Break;
}
...
try { 
  ...
  $o_Notes += "`r`nCannot connect to current Domain."
  $blnWARN = $true
  Break;
}
...
if (!$DCList) {
  ...
  $o_Notes += "`r`nNo Domain Controller found. Run this solution on AD server. Please try again."
  $blnWARN = $true
  Break;
}
...
if ($DCList.Count -eq 0) {
  ...
  $o_Notes += "`r`nAs no machines left script won't continue further"
  Break
}
...
try {
  ...
  $o_Notes += "`r`nGet-ADForest : Could not find a forest identified by: '$($DCtoConnect)'."
}
...
$o_Notes += "`r`nDetermining Domain Password Policy........... "
...
foreach ($item in $props) {
  $flag= 'passed'
  If (($item -eq 'ComplexityEnabled') -and ($DomainPasswordPolicy.ComplexityEnabled -ne 'True')) { $flag = "failed"; $blnWARN = $true }
  If (($item -eq 'MinPasswordLength') -and $DomainPasswordPolicy.MinPasswordLength -le 14) { $flag = "failed"; $blnWARN = $true }
  ...
  If (($item -eq 'PasswordHistoryCount') -and $DomainPasswordPolicy.PasswordHistoryCount -lt '10') { $flag = "failed"; $blnWARN = $true }
  If (($item -eq 'ReversibleEncryptionEnabled') -and $DomainPasswordPolicy.ReversibleEncryptionEnabled -eq 'True') { $flag = "failed"; $blnWARN = $true }
  If (($item -eq 'LockoutThreshold') -and ($DomainPasswordPolicy.LockoutThreshold -gt 10 -or $DomainPasswordPolicy.LockoutThreshold -eq 0)) { $flag = "failed";$blnWARN = $true }
...
$o_Notes += "`r`nChecking Tombstone and Backup Information........"
#Start Sub Container
...
$o_Notes += "`r`nChecking Kerberos delegation Info........"
#Start Container and Sub Container Div
...
$o_Notes += "`r`nScan SYSVOL for Group Policy Preference Passwords......."
#Start Sub Container
...
Get-ChildItem $DomainSYSVOLShareScan -Filter *.xml -Recurse |  % {
  If (Select-String -Path $_.FullName -Pattern "Cpassword") {
    $Passfoundfiles += $_.FullName + "</br>" ; $Count += 1; $flag= "failed"; $blnWARN = $true
  }
}
$gpppwdrow += "<tr>
...
$o_Notes += "`r`nChecking KRBTGT account info........"
#Start Container and Sub Container Div
...
$o_Notes += "`r`nPerforming Privileged AD Group Report......."
#Start Container and Sub Container Div
...
$o_Notes += "`r`nPerforming Misc. User Checks Report......."
...
$o_Notes += "`r`nPlease find the report in C:\IT\Reports directory."
