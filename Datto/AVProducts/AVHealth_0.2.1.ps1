...
.CHANGELOG
    ...
    0.1.9 Optimization and more bugfixes
          ...
          Added '$script:ncxml<vendor>' variables for assigning static 'fallback' sources for AV Product XMLs; XMLs should be uploaded to NC Script Repository and URLs updated (Begin Ln162)
...
#REGION ----- FUNCTIONS ----
  ...
  function Get-AVState {                                                                            #DETERMINE ANTIVIRUS STATE
    ...
    if (-not $script:blnPSXML) {                                                                    #AV PRODUCT STATES NOT LOADED INTO HASHTABLE
      ...
      try {
        $script:defstatus = "Definition Status : $($script:pskey["ps$($state)"].defstatus)"
        $script:rtstatus = "Real-Time Scanning : $($script:pskey["ps$($state)"].rtstatus)"
      } catch {
        $script:defstatus = "Definition Status : Unknown (WMI Check)`r`nAV Product State Unknown : $($state)"
        $script:rtstatus = "Real-Time Scanning : Unknown (WMI Check)`r`nAV Product State Unknown : $($state)"
      }
    }
  } ## Get-AVState
...
if (-not ($script:blnAVXML)) {
  ...
  $script:diag += "`r`nAV Product discovery completed`r`n"
    ...
    foreach ($av in $avs.keys) {                                                                    #ITERATE THROUGH EACH FOUND AV PRODUCT
      if (($avs[$av].display -ne $null) -and ($avs[$av].display -ne "")) {
        ...
        if (($avs[$av].display -notmatch $env:i_PAV) -and ($avs[$av].display -notmatch "Windows Defender")) {
          ...
          Pop-Warnings $script:avwarn $env:i_PAV "AV Health : AV Conflict detected`r`n"
        }
        #PRIMARY AV PRODUCT
        if (($avs[$av].display -match $env:i_PAV) -or 
          ...
          try {
            ...
            $script:diag += "Could not validate Registry data : 'HKLM:$($i_compverkey)' for '$($component.PSChildName)'`r`n$($_.scriptstacktrace)`r`n$($_)`r`n"
          ...
          try {
            ...
            if ($script:zUpgrade -contains $avs[$av].display) {                                     #AV PRODUCTS TREATING '0' AS 'UPTODATE'
              ...
              if ($statkey.$i_statval -eq "0") {
                ...
                Pop-Warnings $script:avwarn $($avs[$av].display) "AV Health : $($script:o_AVStatus)`r`n"
              }
          ...
          try {
            ...
            if ($script:zRealTime -contains $avs[$av].display) {                                    #AV PRODUCTS TREATING '0' AS 'ENABLED' FOR 'REAL-TIME SCANNING'
              ...
              if ($rtkey.$i_rtval -eq 0) {
                $script:o_RTstate = "Real-Time Scanning : Enabled (REG Check)`r`n"
              } elseif ($rtkey.$i_rtval -eq 1) {
                $script:blnWARN = $true
                $script:o_RTstate = "Real-Time Scanning : Disabled (REG Check)`r`n"
                Pop-Warnings $script:avwarn $($avs[$av].display) "AV Health : $($script:o_RTstate)`r`n"
              } else {
                $script:blnWARN = $true
                $script:o_RTstate = "Real-Time Scanning : Unknown (REG Check)`r`n"
                Pop-Warnings $script:avwarn $($avs[$av].display) "AV Health : $($script:o_RTstate)`r`n"
              }
            ...
            $script:o_RTstate = "Real-Time Scanning : N/A (REG Check)`r`n"
            Pop-Warnings $script:avwarn $($avs[$av].display) "AV Health : $($script:o_RTstate)`r`n"
            write-host $_.scriptstacktrace
            write-host $_
          }
          $script:o_AVStatus += "$($script:o_RTstate)"
...
#REAL-TIME SCANNING & DEFINITIONS
$script:diag += "Definitions :`r`nStatus : $($script:o_DefStatus)`r`n"
write-host "Definitions :" -foregroundcolor yellow
write-host "$($script:o_DefStatus)" -foregroundcolor $ccode
#THREATS
$script:diag += "`r`nActive Detections :$($script:o_Infect)`r`n$($script:o_Threats)`r`n"
