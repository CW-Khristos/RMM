...
.CHANGELOG
    ...
    0.1.9 Optimization and more bugfixes
          ...
          Added '$ncxml<vendor>' variables for assigning static 'fallback' sources for AV Product XMLs; XMLs should be uploaded to NC Script Repository and URLs updated (Begin Ln162)
...
#REGION ----- FUNCTIONS ----
  ...
  function Get-AVState {                                                                            #DETERMINE ANTIVIRUS STATE
    ...
    if (-not $blnPSXML) {                                                                    #AV PRODUCT STATES NOT LOADED INTO HASHTABLE
        ...
        try {
          ...
          try {
            ...
            try {
              ...
              write-host "BITS.Transfer() - Could not download $($srcAVP)" -foregroundcolor red
              $defstatus = "Definition Status : Unknown (WMI Check)`r`nUnable to download AV Product State XML"
              $rtstatus = "Real-Time Scanning : Unknown (WMI Check)`r`nUnable to download AV Product State XML"
              $blnPSXML = $false
            }
          }
        }
      ...
      try {
        $defstatus = "Definition Status : $($pskey["ps$($state)"].defstatus)"
        $rtstatus = "Real-Time Scanning : $($pskey["ps$($state)"].rtstatus)"
      } catch {
        $defstatus = "Definition Status : Unknown (WMI Check)`r`nAV Product State Unknown : $($state)"
        $rtstatus = "Real-Time Scanning : Unknown (WMI Check)`r`nAV Product State Unknown : $($state)"
      }
    }
  } ## Get-AVState
      ...
      if (($avs[$av].display -ne $null) -and ($avs[$av].display -ne "")) {
        ...
        if (($avs[$av].display -match $env:i_PAV) -or 
          ...
          try {
            ...
            if ($zRealTime -contains $avs[$av].display) {                                    #AV PRODUCTS TREATING '0' AS 'ENABLED' FOR 'REAL-TIME SCANNING'
              ...
              if ($rtkey.$i_rtval -eq 0) {
                $o_RTstate = "Real-Time Scanning : Enabled (REG Check)`r`n"
              } elseif ($rtkey.$i_rtval -eq 1) {
                $blnWARN = $true
                $o_RTstate = "Real-Time Scanning : Disabled (REG Check)`r`n"
                Pop-Warnings $avwarn $($avs[$av].display) "AV Health : $($o_RTstate)`r`n"
              } else {
                $blnWARN = $true
                $o_RTstate = "Real-Time Scanning : Unknown (REG Check)`r`n"
                Pop-Warnings $avwarn $($avs[$av].display) "AV Health : $($o_RTstate)`r`n"
              }
            ...
            $o_RTstate = "Real-Time Scanning : N/A (REG Check)`r`n"
            Pop-Warnings $avwarn $($avs[$av].display) "AV Health : $($o_RTstate)`r`n"
            write-host $_.scriptstacktrace
            write-host $_
          }
          $o_AVStatus += "$($o_RTstate)"
          ...
          if ($blnWMI) {
            ...
            $o_DefStatus = "Definition Status : N/A (WMI Check)`r`n"
          }
          try {
            ...
            if ($avs[$av].display -match "Windows Defender") {                                      #WINDOWS DEFENDER DEFINITION UPDATE TIMESTAMP
              ...
              if ($age.compareto($time1) -le 0) {
                $o_DefStatus += "Definition Status : Up to date (REG Check)`r`n"
              } elseif ($age.compareto($time1) -gt 0) {
                $o_DefStatus += "Definition Status : Out of date (REG Check)`r`n"
              }
...
write-host "$($o_DefStatus)" -foregroundcolor $ccode
#THREATS
