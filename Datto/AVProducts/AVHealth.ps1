    ...
    if (-not $script:blnPSXML) {                                                                    #AV PRODUCT STATES NOT LOADED INTO HASHTABLE
      ...
      if (test-path "C:\IT\Scripts\productstate.xml") {
        try {
          ...
          write-output "XML.Load() - Could not open C:\IT\Scripts\productstate.xml"
...
elseif ($env:i_PAV -match "BitDefender") {
  ...
  write-output "`r`nBitDefender Selected; will additionally check each AV Product in all Vendor XMLs"
          ...
          if (($avs[$av].display -match $env:i_PAV) -or 
            ...
            try {
              if ($avs[$av].display -match "Sophos") {
                ...
                write-output "Reading : -path 'HKLM:$($i_reboot)' -name '$($i_rebootval1)'"
                ...
                write-output "Reading : -path 'HKLM:$($i_reboot)' -name '$($i_rebootval2)'"
              ...
              if ($avs[$av].display -match "Sophos") {                                      #SOPHOS TREATS '0' AS 'NOT REQUIRED' FOR 'REBOOT REQUIRED'
                ...
                write-output "$($avs[$av].display) reports '$($rval1)' for 'Reboot Required' (Expected : '0')"
                $script:diag += "$($avs[$av].display) reports '$($rval2)' for 'Urgent Reboot Required' (Expected : '0')`r`n"
                write-output "$($avs[$av].display) reports '$($rval2)' for 'Urgent Reboot Required' (Expected : '0')"
              ...
              write-output "Could not validate Registry data : -path 'HKLM:$($i_reboot)' -name '$($i_rebootval1)'"
              $reboot = "Unknown (REG Check)"
              write-output $_.scriptstacktrace
              write-output $_
            }
