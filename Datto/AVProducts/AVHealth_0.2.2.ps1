    ...
    foreach ($av in $avs.keys) {                                                                    #ITERATE THROUGH EACH FOUND AV PRODUCT
      if (($avs[$av].display -ne $null) -and ($avs[$av].display -ne "")) {
        ...
        if (($avs[$av].display -notmatch $env:i_PAV) -and ($avs[$av].display -notmatch "Windows Defender")) {
          ...
          Pop-Warnings $global:avwarn $env:i_PAV "AV Conflict detected`r`n"
        }
        #PRIMARY AV PRODUCT
        if (($avs[$av].display -match $env:i_PAV) -or 
          ...
          if ($rtWARN) {
            $rtWARN = $false
            $global:blnWARN = $true
            Pop-Warnings $global:avwarn $($avs[$av].display) "$($global:o_RTstate)`r`n"
          }
          ...
          if ($tamperWARN) {
            $tamperWARN = $false
            $global:blnWARN = $true
            Pop-Warnings $global:avwarn $($avs[$av].display) "$($tamper)`r`n"
          }
          ...
          if ($scanWARN) {
            $scanWARN = $false
            $global:blnWARN = $true
            Pop-Warnings $global:avwarn $($avs[$av].display) "$($scans)`r`n"
          }
          ...
          if ($defWARN) {
            $defWARN = $false
            $global:blnWARN = $true
            Pop-Warnings $global:avwarn $($avs[$av].display) "$($global:o_DefStatus)`r`n"
          }
