...
if ($strMod -gt $((Get-Date).AddDays(-1))) {
  ...
  Get-Content "$($env:SystemRoot)\System32\drivers\etc\hosts" | ForEach-Object {
    if (($null -ne $_) -and ($_ -ne "")) {
      if (-not ($script:arrHOST)) {
        $script:blnBAK = $false
      } elseif (($script:arrHOST) -and ($script:arrHOST[$script:intLN])) {
        $script:blnBAK = $true
  ...
  if ($script:blnBAK -and $script:blnCHG) {
    ...
    if ($script:blnBAK) {
      ...
      $script:diag += "`r`nFILE MODIFIED; BUT NO BACKUP FOR COMPARISON`r`n"
  ...
  #WRITE HOSTS BACKUP
  write-output "CREATING HOSTS BACKUP :"
  $script:diag += "CREATING HOSTS BACKUP :`r`n"
  Copy-Item "$($env:SystemRoot)\System32\drivers\etc\hosts" -Destination "C:\IT\hosts" -force
  write-output "`tDONE"
  $script:diag += "`tDone`r`n"
